...
// After the entire input string has been converted the result should be "********".
int main(void)
{
    char     atoi_input[4];
    ...
    if (scanf_ret != 1 || ascii_values[0] != '4' || ascii_values[1] != '2')
        no();
    ...
    // Initialize the converted string with '*' at the start since the first char will be skipped 
    converted_input[0] = '*';
    
    // The first 2 chars from the user input have to be '4' and '2', so we skip those
    user_input_index = 2;

    // We skip the first char since it is already initialized as '*'
    ascii_str_index = 1;
    ...
    // Compare the converted input with the password "********"
    if (strcmp(converted_input, "********") == 0)
        ok();
    else
        no();
